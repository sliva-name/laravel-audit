...
final class PatternHypothesisSelectionTest extends TestCase
{
    ...
    public function test_report_merger_keeps_heuristics_when_llm_does_not_return_replacement(): void
    {
        $reviewedKey = 'action:app/Http/Controllers/UserController.php::store';
        $missingKey = 'repository:app/Services/UserService.php::create';

        $merged = PatternReportMerger::merge(
            [
                [
                    'hypothesisKey' => $reviewedKey,
                    'pattern' => 'action',
                    'source' => 'heuristic',
                    'title' => 'Action / Use Case',
                ],
        ...
        $this->assertSame('repository', $merged[0]['pattern'] ?? null);
        $this->assertSame('confirmed', $merged[1]['source'] ?? null);
        $this->assertSame('action', $merged[1]['pattern'] ?? null);
    }
}
